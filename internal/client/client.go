...
type Creator func() (cli_service.TCLIService, error)

func TClientCreator(cfg *config.Config, httpclient *http.Client) Creator {
	return func() (cli_service.TCLIService, error) {
		return InitThriftClient(cfg, nil)
	}
}
...
func RetryableClient(cfg *config.Config) *http.Client {
	httpclient := PooledClient(cfg.Authenticator)
...
func PooledClient(authr auth.Authenticator) *http.Client {
	if authr == nil {
		return nil
	}
	tr := &Transport{
