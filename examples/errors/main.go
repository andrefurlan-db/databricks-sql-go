...
func accessTokenInvalid() {
	...
	connector, err := dbsql.NewConnector(
		...
		dbsql.WithInitialNamespace("andre", "main"),
	)
	if err != nil {
		log.Fatal(err)
	}
...
func userDoesNotHavePermission() {
	...
	connector, err := dbsql.NewConnector(
		...
		dbsql.WithAccessToken(os.Getenv("DATABRICKS_ACCESSTOKEN")),
		dbsql.WithUserAgentEntry("workflow-example"),
		dbsql.WithInitialNamespace("andre", "default"),
	)
	if err != nil {
		log.Fatal(err)
	}
...
func invalidSchema2() {
	...
	connector, err := dbsql.NewConnector(
		...
		dbsql.WithAccessToken(os.Getenv("DATABRICKS_ACCESSTOKEN")),
		dbsql.WithUserAgentEntry("workflow-example"),
		dbsql.WithInitialNamespace("andre", "invalid"),
	)
	if err != nil {
		log.Fatal(err)
	}
	...
	// Exec
	if _, err := db.ExecContext(context.Background(), `select * from foo`); err != nil {
		log.Fatal(err)
	}
}
...
func tableIdentifierIsInvalid() {
	...
	// Query row
	var max float64
	if err := db.QueryRowContext(context.Background(), "select max(carat) from diamonds-invalid").Scan(&max); err != nil {
		log.Fatal(err)
	} else {
...
func invalidCatalog() {
	connector, err := dbsql.NewConnector(
		...
		dbsql.WithAccessToken(os.Getenv("DATABRICKS_ACCESSTOKEN")),
		dbsql.WithUserAgentEntry("workflow-example"),
		dbsql.WithInitialNamespace("andre-invalid", "default"),
	)
	if err != nil {
		log.Fatal(err)
	}
...
func warehouseNotStarted() {
	connector, err := dbsql.NewConnector(
		dbsql.WithServerHostname(os.Getenv("DATABRICKS_HOST")),
		dbsql.WithHTTPPath("/sql/1.0/warehouses/5c89f447c476a5a8"), //bi-load-test
		dbsql.WithAccessToken(os.Getenv("DATABRICKS_ACCESSTOKEN")),
		dbsql.WithUserAgentEntry("workflow-example"),
		dbsql.WithInitialNamespace("andre", "default"),
		dbsql.WithRetries(-1, 0, 1*time.Minute),
	)
	if err != nil {
		log.Fatal(err)
	}
...
func queryCanceled() {
	...
	// Query row
	var max float64
	if err := db.QueryRowContext(context.Background(), "SELECT id FROM RANGE(1000000000) ORDER BY RANDOM() + 2 asc").Scan(&max); err != nil {
		log.Fatal(err)
	} else {
		fmt.Printf("max carat in dataset is: %f\n", max)
	}
...
func timeout() {
	...
	// Query row
	var max float64
	if err := db.QueryRowContext(ctx1, "SELECT id FROM RANGE(1000000000) ORDER BY RANDOM() + 2 asc").Scan(&max); err != nil {
		log.Fatal(err)
	} else {
		fmt.Printf("max carat in dataset is: %f\n", max)
	}
...
func timeout2() {
	connector, err := dbsql.NewConnector(
		...
		dbsql.WithUserAgentEntry("workflow-example"),
		dbsql.WithInitialNamespace("andre", "default"),
		dbsql.WithTimeout(5*time.Second),
	)
	if err != nil {
		log.Fatal(err)
	}
	db := sql.OpenDB(connector)
	defer db.Close()
	ctx1, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
...
func invalidDsnFormat() {
	...
	{
		dsn := fmt.Sprintf("token:%s@%s:%d", os.Getenv("DATABRICKS_ACCESSTOKEN"), os.Getenv("DATABRICKS_HOST"), 433)
		_, err := sql.Open("databricks", dsn)
		if err != nil {
			log.Println(err)
		}
